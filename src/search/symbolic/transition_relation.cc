...
TransitionRelation::TransitionRelation(SymVariables *sVars, std::shared_ptr<SymAxiomEvaluator> axiomEvaluator,
				       const GlobalOperator *op, int cost_) :
    sV(sVars), axiomEvaluator(axiomEvaluator), cost(cost_), tBDD(sVars->oneBDD()),
    ...
    for (int i = op->get_effects().size() - 1; i >= 0; i--) {
        ...
        for (const auto &cPrev : effect.conditions) {
            condition *= getPreCondition(cPrev);
        }
        effect_conditions[var] *= !condition;
    ...
    //sV->to_dot(tBDD, op->get_name());
}

BDD TransitionRelation::getPreCondition(const GlobalCondition& cond) const {
	BDD res;
        if (axiomEvaluator->is_derived_variable(cond.var)) {
		res = axiomEvaluator->derived_representation(cond.var, cond.val);
	} else {
		res = sV->preBDD(cond.var, cond.val);
	}
	return res;
}
