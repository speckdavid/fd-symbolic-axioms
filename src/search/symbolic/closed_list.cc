...
void ClosedList::insert(int h, const BDD &S) {
	DEBUG_MSG(
			cout << "Inserting on closed " << "g=" << h << ": " << S.nodeCount()
					<< " nodes and " << mgr->getVars()->numStates(S)
					<< " states" << endl
			;
	);
...
void ClosedList::extract_path(const BDD &c, int h, bool fw,
	...
	if (!mgr)
		return;
	DEBUG_MSG(
			cout << "Sym closed extract path h=" << h << " notClosed: " << hNotClosed << endl; cout << "Closed: "; for (auto &c : closed) cout << c.first << " "; cout << endl;);
	const map<int, vector<TransitionRelation>> &trs = mgr->getIndividualTRs();
	BDD cut = c;
	if (has_axioms()) {
		mgr->get_axiom_evaluator()->abstract_derived(cut, cut);
	}
	size_t steps0 = 0;
	if (zeroCostClosed.count(h)) {
		assert(trs.count(0));
		// DEBUG_MSG(cout << "Check " << steps0 << " of " <<
		// zeroCostClosed.at(h).size() << endl;);
		while (steps0 < zeroCostClosed.at(h).size()
				&& (cut * zeroCostClosed.at(h)[steps0]).IsZero()) {
			// DEBUG_MSG(cout << "Steps0 is not " << steps0<< " of " <<
			// zeroCostClosed.at(h).size() << endl;);
			steps0++;
		}
		...
		if (steps0 < zeroCostClosed.at(h).size()) {
			...
			DEBUG_MSG(
					cout
							<< "cut not found with steps0. Try to find with preimage: "
							<< trs.count(0) << endl
					;
			);
			...
			for (const TransitionRelation &tr : trs.at(0)) {
				...
				for (size_t newSteps0 = 0; newSteps0 < steps0; newSteps0++) {
					...
					if (!intersection.IsZero()) {
						...
						// DEBUG_MSG(cout << "Adding " <<
						// (*(tr.getOps().begin()))->get_name() << endl;);
		...
		if (steps0 > 0) {
			...
			if (trs.count(0)) {
				for (const TransitionRelation &tr : trs.at(0)) {
					...
					for (size_t newSteps0 = 0; newSteps0 < steps0;
							newSteps0++) {
						BDD intersection = succ
								* zeroCostClosed.at(h)[newSteps0];
						if (!intersection.IsZero()) {
							steps0 = newSteps0;
							cut = intersection;
							if (has_axioms()) {
			...
			if (!foundZeroCost) {
				/*    DEBUG_MSG(
				 ...
				 zeroCostClosed.at(h)[newSteps0].print(0,2); cout << "CUT: ";
				 cut.print(0, 1);
				 }
				 );*/
				steps0 = 0;
					...
					if (!intersection.IsZero()) {
						...
						if (zeroCostClosed.count(h)) {
							while ((cut * zeroCostClosed.at(h)[steps0]).IsZero()) {
								// DEBUG_MSG(cout << "r Steps0 is not " << steps0<< " of " <<
								// zeroCostClosed.at(h).size() << endl;);
								steps0++;
								assert(steps0 < zeroCostClosed.at(newH).size());
							}

							// DEBUG_MSG(cout << "r Steps0 of h=" << h << " is " << steps0 <<
							// endl;);
						...
						// DEBUG_MSG(cout << "Selected " << path.back()->get_name() <<
						// endl;);
...
if (closed.size() <= 1 && hNotClosed <= previousMaxH) {
	cout << "Heuristic not inserted: " << hNotClosed << " " << closed.size()
			<< endl;
	return;
}
...
cout << "Heuristic with maxValue: " << hNotClosed << " ADD size: "
		<< h.nodeCount() << endl;
...
if (hNotClosed != numeric_limits<int>::max() && hNotClosed >= 0
		&& !statesWithHNotClosed.IsZero()) {
...
// cout << "h (eval " << num_calls_eval << ", not_closed" << time_eval_states
...
DEBUG_MSG(
		cout << maxH << " " << notClosedSize << endl; cout << "Max size: " << heuristicSize << endl << endl;);
