...
extern void memoryOutError(size_t size);
...
class SymVariables {
  ...
  std::unique_ptr<Cudd>
      _manager;  //_manager associated with this symbolic search
 ...
 public:
  ...
  double numStates(
      const BDD &bdd) const;  // Returns the number of states in a BDD
  ...
  inline long totalNodes() const { return _manager->ReadNodeCount(); }
  ...
  inline BDD zeroBDD() const { return _manager->bddZero(); }
  ...
  inline Cudd *mgr() const { return _manager.get(); }

  inline BDD bddVar(int index) const { return variables[index]; }

  inline int usedNodes() const { return _manager->ReadSize(); }
  ...
  inline void unsetTimeLimit() { _manager->UnsetTimeLimit(); }
  ...
  inline ADD getADD(int value) { return _manager->constant(value); }
