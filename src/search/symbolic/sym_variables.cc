...
void SymVariables::init(const vector<int> &v_order) {
  ...
  _manager->RegisterOutOfMemoryCallback(Cudd_OutOfMemSilent);
...
double SymVariables::numStates() const { return numStates(validBDD); }
...
void SymVariables::print() {
  ...
  for (int v : var_order) {
    ...
    file << endl;
    for (auto fact : g_fact_names[v]) file << fact << endl;
  }
}
...
void SymVariables::to_dot(const BDD &bdd, const std::string file_name) const {
  std::vector<string> var_names(numBDDVars * 2);
  for (int v : var_order) {
    int exp = 0;
    for (int j : bdd_index_pre[v]) {
      var_names[j] = g_variable_name[v] + "_2^" + std::to_string(exp);
      var_names[j + 1] =
          g_variable_name[v] + "_2^" + std::to_string(exp++) + "_primed";
    }
  }
  ...
  Cudd_DumpDot(mgr()->getManager(), 1, ddnodearray, names.data(), NULL,
...
void SymVariables::add_options_to_parser(options::OptionParser &parser) {
  ...
  //                     "variable ordering for the symbolic manager",
  //                     "REVERSE_LEVEL");
