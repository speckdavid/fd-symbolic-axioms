...
OriginalStateSpace::OriginalStateSpace(
  ...
  if (has_axioms()) {
      assert(axiomEvaluator);
      axiomEvaluator->init(v, &params);
    axiomEvaluator->init_axioms();
    axiomEvaluator->process_init_state(initialState, initialState);
    axiomEvaluator->process_goal_state(goal, goal);
    // vars->to_dot(initialState, "init.dot");
    // vars->to_dot(goal, "goal.dot");
  }
  ...
  for (size_t i = 0; i < g_operators.size(); i++) {
    ...
    DEBUG_MSG(cout << "Creating TR of op " << i << " of cost " << cost
                   << endl;);
    indTRs[cost].push_back(TransitionRelation(vars, axiomEvaluator, op, cost));
    if (has_axioms() && indTRs[cost].back().is_empty()) {
      indTRs[cost].pop_back();
      empty_trs++;
    } else {
  ...
  std::cout << "Unapplicable operators: " << empty_trs << std::endl;
  ...
  for (auto &mg : mutex_groups) {
    ...
    if (mg.isExactlyOne()) {
      ...
      for (auto &fluent : invariant_group) {
        ...
        bddInvariant += getPreCondition(fluent.var, fluent.value);
        if (fluent.var < var) {
          var = fluent.var;
          val = fluent.value;
        }
    ...
    for (size_t i = 0; i < invariant_group.size(); ++i) {
      ...
      BDD f1 = getPreCondition(var1, val1);
      ...
      for (size_t j = i + 1; j < invariant_group.size(); ++j) {
        ...
        BDD f2 = getPreCondition(var2, val2);
        BDD mBDD = !(f1 * f2);
        if (genMutexBDD) {
          num_mutex++;
...
BDD OriginalStateSpace::getPreCondition(int var, int val) const {
  BDD res;
  if (axiomEvaluator->is_derived_variable(var)) {
    res = axiomEvaluator->derived_representation(var, val);
  } else {
