...
SymStateSpaceManager::SymStateSpaceManager(SymVariables *v,
    ...
    : vars(v),
      p(params),
      relevant_vars(relevant_vars_),
      initialState(v->zeroBDD()),
      goal(v->zeroBDD()),
      min_transition_cost(0),
      hasTR0(false),
      axiomEvaluator(params.axiom_evaluator) {
...
void SymStateSpaceManager::addDeadEndStates(
    const std::vector<BDD> &fw_dead_ends,
...
void SymStateSpaceManager::zero_preimage(const BDD &bdd, vector<BDD> &res,
  ...
  // Apply axioms if necessary
  if (!has_axioms()) {
    return;
  }
  for (auto &b : res) {
    axiomEvaluator->apply_axioms(b, b);
  }
}
...
void SymStateSpaceManager::cost_image(const BDD &bdd,
  ...
  for (auto trs : transitions) {
    ...
    if (cost == 0) continue;
    for (size_t i = res[cost].size(); i < trs.second.size(); i++) {
      // cout << "Img: " << trs.second[i].nodeCount() << " with bdd " <<
      // bdd.nodeCount() << " node limit: " << nodeLimit << endl;
...
BDD SymStateSpaceManager::filter_mutex(const BDD &bdd, bool fw, int nodeLimit,
  ...
  switch (p.mutex_type) {
    ...
    case MutexType::MUTEX_EDELETION:
      if (initialization) {
        for (const BDD &notMutexBDD : notMutexBDDs) {
          DEBUG_MSG(cout << res.nodeCount() << " and "
    ...
    case MutexType::MUTEX_NPAND:
      for (const BDD &notMutexBDD : notMutexBDDs) res = res.NPAnd(notMutexBDD);
      break;
...
SymParamsMgr::SymParamsMgr(const options::Options &opts)
    : max_tr_size(opts.get<int>("max_tr_size")),
      ...
      axiom_evaluator(opts.get<shared_ptr<SymAxiomEvaluator>>("axiom_type")),
...
void SymParamsMgr::print_options() const {
  ...
  cout << "Axiom(" << axiom_evaluator->get_name() << ")" << endl;
...
void SymParamsMgr::add_options_to_parser(options::OptionParser &parser) {
  ...
  parser.add_option<shared_ptr<SymAxiomEvaluator>>("axiom_type",  "axiom evaluator", "transform");
