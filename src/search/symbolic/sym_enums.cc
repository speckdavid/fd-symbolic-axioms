...
std::ostream &operator<<(std::ostream &os, const AxiomType &a) {
  switch (a) {
    case AxiomType::INTERLEAVED:
      return os << "interleaved";
  case AxiomType::INTERLEAVED_RULES:
      return os << "interleaved rules (option 1)";
    case AxiomType::PRECOMPUTED:
      return os << "precomputed (mix of option 1 and option 2)";
    case AxiomType::VARBDDS:
      return os << "varbdds (option 2)";
    case AxiomType::TRANSFORM:
      return os << "transform (option 3)";
    default:
      std::cerr << "Name of AxiomType not known";
      utils::exit_with(utils::ExitCode::UNSUPPORTED);
  }
}

    const std::vector<std::string> AxiomTypeValues{"INTERLEAVED", "INTERLEAVED_RULES", "PRECOMPUTED",
                                               "VARBDDS", "TRANSFORM"};


// std::ostream &operator<<(std::ostream &os, const AxiomStratification &a) {
//   switch (a) {
//     case AxiomStratification::DEFAULT:
//       return os << "FD stratification (if valid)";
//     case AxiomStratification::TRANSITIVE_CLOSURE:
//       return os << "Transitive closure (\"In defense of axioms\")";
//     default:
//       std::cerr << "Name of AxiomStratification not known";
//       utils::exit_with(utils::ExitCode::UNSUPPORTED);
//   }
// }

const std::vector<std::string> AxiomStratificationValues{"DEFAULT",
                                                         "TRANSITIVE_CLOSURE"};

const std::vector<std::string> MutexTypeValues{
    "MUTEX_NOT",       "MUTEX_AND",    "MUTEX_RESTRICT", "MUTEX_NPAND",
    "MUTEX_CONSTRAIN", "MUTEX_LICOMP", "MUTEX_EDELETION"};

const std::vector<std::string> LinearPDBStrategyValues{"LEVEL", "REVERSE",
                                                       "BINARY"};

const std::vector<std::string> AbsTRsStrategyValues{
    "TR_SHRINK", "IND_TR_SHRINK", "REBUILD_TRS", "SHRINK_AFTER_IMG"};

const std::vector<std::string> RelaxDirStrategyValues{"FW", "BW", "BIDIR",
                                                      "SWITCHBACK"};
...
const std::vector<std::string> UCTRewardTypeValues{
    "STATES",       "NODES", "STATES_TIME", "NODES_TIME",
    "STATES_NODES", "NONE",  "RANDOM"};
}  // namespace symbolic
